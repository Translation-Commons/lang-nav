### Test Coverage (Vitest)

| Path                                              | Lines | Branches | Funcs | Stmts |
| ------------------------------------------------- | ----: | -------: | ----: | ----: |
| `src`                                             |  35.5 |     78.5 |  54.6 |  35.5 |
| `  app`                                           |     0 |        0 |     0 |     0 |
| `    App.tsx`                                     |     0 |        0 |     0 |     0 |
| `    PageRoutes.tsx`                              |     0 |        0 |     0 |     0 |
| `  entities`                                      |  40.1 |     81.8 |  44.8 |  40.1 |
| `    census`                                      |  42.9 |     40.0 |  50.0 |  42.9 |
| `      CensusCard.tsx`                            |  19.2 |    100.0 |     0 |  19.2 |
| `      CensusTypes.tsx`                           |  81.3 |     40.0 | 100.0 |  81.3 |
| `    language`                                    |  35.7 |     93.4 |  35.2 |  35.7 |
| `      LanguageCanonicalName.tsx`                 |  14.3 |    100.0 |     0 |  14.3 |
| `      LanguageCard.tsx`                          |  22.8 |    100.0 |     0 |  22.8 |
| `      LanguageOtherNames.tsx`                    |  26.3 |    100.0 |     0 |  26.3 |
| `      LanguagePopulationEstimate.tsx`            |  27.8 |    100.0 |     0 |  27.8 |
| `      LanguagePopulationFromDescendants.tsx`     |   9.7 |    100.0 |     0 |   9.7 |
| `      LanguagePopulationFromLocales.tsx`         |  15.2 |    100.0 |     0 |  15.2 |
| `      LanguagePopulationInSelectedTerritory.tsx` |     0 |        0 |     0 |     0 |
| `      LanguageRetirementReason.tsx`              |  14.3 |    100.0 |     0 |  14.3 |
| `      LanguageTypes.ts`                          | 100.0 |    100.0 | 100.0 | 100.0 |
| `      LanguageWritingSystems.tsx`                |     0 |    100.0 | 100.0 |     0 |
| `      plurals`                                   |  34.4 |     95.2 |  36.0 |  34.4 |
| `        LanguagePluralCategories.tsx`            |  24.2 |    100.0 |     0 |  24.2 |
| `        LanguagePluralComputation.ts`            |  97.1 |     96.4 | 100.0 |  97.1 |
| `        LanguagePluralGrid.tsx`                  |  24.6 |    100.0 |     0 |  24.6 |
| `        LanguagePluralGridToggle.tsx`            |  25.9 |    100.0 |     0 |  25.9 |
| `        LanguagePluralRuleSymbolsUsed.tsx`       |     0 |        0 |     0 |     0 |
| `        PluralRuleExplanation.tsx`               |   8.3 |    100.0 |     0 |   8.3 |
| `        PluralRuleSymbolExplanation.tsx`         |   9.4 |    100.0 |     0 |   9.4 |
| `        PluralStrings.ts`                        |   9.1 |    100.0 |     0 |   9.1 |
| `      setLanguageNames.ts`                       | 100.0 |    100.0 | 100.0 | 100.0 |
| `      vitality`                                  |  43.4 |     91.7 |  46.7 |  43.4 |
| `        LanguageDetailsVitalityAndViability.tsx` |  20.4 |    100.0 |     0 |  20.4 |
| `        LanguageVitalityCell.tsx`                |     0 |        0 |     0 |     0 |
| `        LanguageVitalityComputation.tsx`         |  14.3 |    100.0 |  16.7 |  14.3 |
| `        VitalityMeter.tsx`                       |  21.7 |    100.0 |     0 |  21.7 |
| `        VitalityParsing.ts`                      |  98.6 |     93.0 | 100.0 |  98.6 |
| `        VitalityStrings.ts`                      |  95.1 |     88.0 | 100.0 |  95.1 |
| `        VitalityTypes.ts`                        | 100.0 |    100.0 | 100.0 | 100.0 |
| `    lib`                                         |  87.3 |     77.9 |  90.0 |  87.3 |
| `      getObjectFromID.ts`                        |  22.2 |    100.0 |     0 |  22.2 |
| `      getObjectMiscFields.ts`                    |  99.1 |     80.3 | 100.0 |  99.1 |
| `      getObjectName.ts`                          |  53.2 |     41.7 |  66.7 |  53.2 |
| `      getObjectPopulation.ts`                    |  94.6 |     82.0 | 100.0 |  94.6 |
| `    locale`                                      |  18.5 |     80.8 |  40.0 |  18.5 |
| `      LocaleCard.tsx`                            |  19.4 |    100.0 |     0 |  19.4 |
| `      LocaleCensusCitation.tsx`                  |   8.8 |    100.0 |     0 |   8.8 |
| `      LocaleCitationCounts.tsx`                  |     0 |    100.0 | 100.0 |     0 |
| `      LocaleNameWithFilters.tsx`                 |     0 |    100.0 | 100.0 |     0 |
| `      LocalePopulationAdjusted.tsx`              |  10.8 |    100.0 |     0 |  10.8 |
| `      LocaleStrings.tsx`                         |  77.5 |     82.6 |  80.0 |  77.5 |
| `      PotentialLocaleThreshold.tsx`              |     0 |        0 |     0 |     0 |
| `    territory`                                   |  87.2 |     75.0 |  50.0 |  87.2 |
| `      TerritoryCard.tsx`                         |  87.2 |     75.0 |  50.0 |  87.2 |
| `    ui`                                          |  47.7 |     53.3 |  33.3 |  47.7 |
| `      CLDRCoverageInfo.tsx`                      |  15.8 |    100.0 |     0 |  15.8 |
| `      CLDRCoverageLevels.tsx`                    |   7.9 |    100.0 |     0 |   7.9 |
| `      CLDRWarningNotes.tsx`                      |  20.0 |    100.0 |     0 |  20.0 |
| `      ICUSupportStatus.tsx`                      |  25.0 |    100.0 |     0 |  25.0 |
| `      ObjectCard.tsx`                            |  79.2 |     16.7 | 100.0 |  79.2 |
| `      ObjectField.tsx`                           |  83.9 |     54.5 | 100.0 |  83.9 |
| `      ObjectSuggestions.tsx`                     |  88.9 |     55.5 | 100.0 |  88.9 |
| `      ObjectTitle.tsx`                           |  87.8 |     75.0 | 100.0 |  87.8 |
| `      ObjectWikipediaInfo.tsx`                   |  17.2 |    100.0 |     0 |  17.2 |
| `    varianttag`                                  |  20.0 |    100.0 |     0 |  20.0 |
| `      VariantTagCard.tsx`                        |  20.0 |    100.0 |     0 |  20.0 |
| `    writingsystem`                               |  12.3 |    100.0 |     0 |  12.3 |
| `      WritingSystemCard.tsx`                     |  12.3 |    100.0 |     0 |  12.3 |
| `  features`                                      |  52.5 |     79.7 |  65.4 |  52.5 |
| `    __tests__`                                   |  89.8 |     92.3 |  75.0 |  89.8 |
| `      MockObjects.tsx`                           |  89.8 |     92.3 |  75.0 |  89.8 |
| `    data`                                        |  32.3 |     71.2 |  65.6 |  32.3 |
| `      compute`                                   |  99.6 |     65.5 | 100.0 |  99.6 |
| `        computeDescedantPopulation.ts`           | 100.0 |     75.0 | 100.0 | 100.0 |
| `        computeLocalePopulationFromCensuses.ts`  |  97.7 |     84.2 | 100.0 |  97.7 |
| `        computeLocaleWritingPopulation.ts`       | 100.0 |     58.3 | 100.0 | 100.0 |
| `        computeRegionalLocalePopulation.ts`      | 100.0 |     57.1 | 100.0 | 100.0 |
| `        computeTerritoryStats.ts`                | 100.0 |     54.2 | 100.0 | 100.0 |
| `        updateObjectCodesNameAndPopulation.ts`   | 100.0 |     73.3 | 100.0 | 100.0 |
| `      connect`                                   |  74.5 |     63.6 |  90.0 |  74.5 |
| `        connectCensuses.ts`                      |  79.0 |     90.9 | 100.0 |  79.0 |
| `        connectLanguages.ts`                     |     0 |        0 |     0 |     0 |
| `        connectLanguagesToParent.ts`             | 100.0 |     83.3 | 100.0 | 100.0 |
| `        connectLocales.ts`                       |  92.7 |     53.8 | 100.0 |  92.7 |
| `        connectTerritoriesToParent.ts`           |  65.4 |     80.0 | 100.0 |  65.4 |
| `        connectWritingSystems.ts`                |  71.7 |     35.7 | 100.0 |  71.7 |
| `        createRegionalLocales.ts`                |  98.6 |     66.7 | 100.0 |  98.6 |
| `      context`                                   |  14.0 |        0 |     0 |  14.0 |
| `        DataProvider.tsx`                        |     0 |        0 |     0 |     0 |
| `        useDataContext.tsx`                      |  81.8 |    100.0 |     0 |  81.8 |
| `      load`                                      |  13.3 |     86.0 |  56.0 |  13.3 |
| `        CoreData.tsx`                            |     0 |        0 |     0 |     0 |
| `        entities`                                |  98.1 |     96.9 | 100.0 |  98.1 |
| `          loadLanguages.ts`                      | 100.0 |    100.0 | 100.0 | 100.0 |
| `          loadLocales.ts`                        |  91.9 |     91.7 | 100.0 |  91.9 |
| `          loadObjectsFromFile.ts`                | 100.0 |    100.0 | 100.0 | 100.0 |
| `          loadTerritories.ts`                    | 100.0 |     80.0 | 100.0 | 100.0 |
| `          loadWritingSystems.ts`                 | 100.0 |    100.0 | 100.0 | 100.0 |
| `        extra_entities`                          |   5.0 |     50.0 |  11.1 |   5.0 |
| `          GlottologData.tsx`                     |     0 |        0 |     0 |     0 |
| `          IANAData.tsx`                          |  24.8 |     70.0 |  20.0 |  24.8 |
| `          ISOData.tsx`                           |     0 |        0 |     0 |     0 |
| `          ISORetirements.tsx`                    |     0 |        0 |     0 |     0 |
| `          loadCensusData.tsx`                    |     0 |        0 |     0 |     0 |
| `        supplemental`                            |     0 |     80.0 |  80.0 |     0 |
| `          loadCountryCoordinates.ts`             |     0 |    100.0 | 100.0 |     0 |
| `          loadLandArea.ts`                       |     0 |    100.0 | 100.0 |     0 |
| `          loadTerritoryGDPLiteracy.ts`           |     0 |    100.0 | 100.0 |     0 |
| `          UnicodeData.tsx`                       |     0 |        0 |     0 |     0 |
| `          WikipediaData.tsx`                     |     0 |    100.0 | 100.0 |     0 |
| `        SupplementalData.tsx`                    |     0 |        0 |     0 |     0 |
| `    hovercard`                                   |  97.6 |     86.8 |  90.0 |  97.6 |
| `      EmptyHoverCardProvider.tsx`                | 100.0 |    100.0 |  50.0 | 100.0 |
| `      Hoverable.tsx`                             | 100.0 |    100.0 | 100.0 | 100.0 |
| `      HoverableButton.tsx`                       | 100.0 |    100.0 | 100.0 | 100.0 |
| `      HoverableEnumeration.tsx`                  | 100.0 |     87.5 | 100.0 | 100.0 |
| `      HoverableObject.tsx`                       |  88.2 |     80.0 | 100.0 |  88.2 |
| `      HoverableObjectName.tsx`                   | 100.0 |     80.0 | 100.0 | 100.0 |
| `      HoverCardContext.tsx`                      | 100.0 |    100.0 | 100.0 | 100.0 |
| `      HoverCardProvider.tsx`                     |  97.6 |     80.0 | 100.0 |  97.6 |
| `      useHoverCard.tsx`                          | 100.0 |    100.0 | 100.0 | 100.0 |
| `    map`                                         |  37.0 |    100.0 |  25.0 |  37.0 |
| `      getRobinsonCoordinates.ts`                 | 100.0 |    100.0 | 100.0 | 100.0 |
| `      MapCircles.tsx`                            |  14.6 |    100.0 |     0 |  14.6 |
| `      ObjectMap.tsx`                             |  28.6 |    100.0 |     0 |  28.6 |
| `    modal`                                       |     0 |        0 |     0 |     0 |
| `      ViewModal.tsx`                             |     0 |        0 |     0 |     0 |
| `    pagination`                                  |  64.3 |     64.7 |  46.7 |  64.3 |
| `      LimitInput.tsx`                            |     0 |    100.0 | 100.0 |     0 |
| `      LimitSelector.tsx`                         |  26.2 |    100.0 |     0 |  26.2 |
| `      PaginationControls.tsx`                    |  88.9 |     50.0 |  14.3 |  88.9 |
| `      usePagination.tsx`                         | 100.0 |     83.3 | 100.0 | 100.0 |
| `      VisibleItemsMeter.tsx`                     |  83.7 |     61.9 |  80.0 |  83.7 |
| `    params`                                      |  59.2 |     73.9 |  65.0 |  59.2 |
| `      getNewURL.ts`                              |     0 |        0 |     0 |     0 |
| `      getNewURLSearchParams.ts`                  |     0 |        0 |     0 |     0 |
| `      getParamsFromURL.ts`                       |     0 |        0 |     0 |     0 |
| `      PageParamsContext.tsx`                     | 100.0 |    100.0 | 100.0 | 100.0 |
| `      PageParamsProvider.tsx`                    |     0 |        0 |     0 |     0 |
| `      PageParamTypes.tsx`                        | 100.0 |    100.0 | 100.0 | 100.0 |
| `      Profiles.tsx`                              |  59.9 |     20.0 |  33.3 |  59.9 |
| `      ui`                                        |  86.8 |     81.9 |  80.6 |  86.8 |
| `        Selector.tsx`                            |  98.0 |     94.1 | 100.0 |  98.0 |
| `        SelectorDisplayContext.tsx`              | 100.0 |    100.0 |  66.7 | 100.0 |
| `        SelectorDropdown.tsx`                    | 100.0 |    100.0 | 100.0 | 100.0 |
| `        SelectorLabel.tsx`                       |  86.7 |     57.1 | 100.0 |  86.7 |
| `        SelectorOption.tsx`                      |  70.4 |     69.2 |  66.7 |  70.4 |
| `        TextInput.tsx`                           |  86.4 |     82.6 |  70.0 |  86.4 |
| `      usePageParams.tsx`                         |  40.0 |    100.0 |     0 |  40.0 |
| `      useStoredParams.tsx`                       |     0 |        0 |     0 |     0 |
| `    table`                                       |  84.3 |     88.7 |  80.0 |  84.3 |
| `      BaseObjectTable.tsx`                       |  89.6 |     57.1 | 100.0 |  89.6 |
| `      CommonColumns.tsx`                         |  96.5 |    100.0 |  66.7 |  96.5 |
| `      InteractiveObjectTable.tsx`                |  88.1 |     87.5 | 100.0 |  88.1 |
| `      TableColumn.tsx`                           | 100.0 |    100.0 | 100.0 | 100.0 |
| `      TableColumnSelector.tsx`                   |  94.3 |     86.4 | 100.0 |  94.3 |
| `      TableExport.tsx`                           |  76.1 |     79.2 |  75.0 |  76.1 |
| `      TableID.tsx`                               | 100.0 |    100.0 | 100.0 | 100.0 |
| `      TableSortButton.tsx`                       | 100.0 |    100.0 | 100.0 | 100.0 |
| `      TableValueType.ts`                         | 100.0 |    100.0 | 100.0 | 100.0 |
| `      useColumnVisibility.tsx`                   |  64.9 |    100.0 |  57.1 |  64.9 |
| `    transforms`                                  |  54.0 |     85.1 |  69.6 |  54.0 |
| `      coloring`                                  |   8.8 |    100.0 |  18.2 |   8.8 |
| `        BaseColorBar.tsx`                        |  11.8 |    100.0 |     0 |  11.8 |
| `        ColorBar.tsx`                            |   7.4 |    100.0 |     0 |   7.4 |
| `        ColorBySelector.tsx`                     |     0 |    100.0 | 100.0 |     0 |
| `        ColorGradientSelector.tsx`               |     0 |    100.0 | 100.0 |     0 |
| `        ColorTypes.ts`                           | 100.0 |    100.0 | 100.0 | 100.0 |
| `        getColorGradientFunction.tsx`            |   4.4 |    100.0 |     0 |   4.4 |
| `        useColors.tsx`                           |   6.4 |    100.0 |     0 |   6.4 |
| `      filtering`                                 |  73.0 |     83.3 |  78.4 |  73.0 |
| `        filter.tsx`                              |  97.0 |     70.2 | 100.0 |  97.0 |
| `        FilterBreakdown.tsx`                     | 100.0 |    100.0 | 100.0 | 100.0 |
| `        filterByConnections.tsx`                 |  92.7 |     86.4 | 100.0 |  92.7 |
| `        FilterPath.tsx`                          |  79.1 |     87.0 |  54.5 |  79.1 |
| `        LanguageFilterSelector.tsx`              |  75.0 |    100.0 |  66.7 |  75.0 |
| `        LanguageScopeSelector.tsx`               |     0 |    100.0 | 100.0 |     0 |
| `        TerritoryFilterSelector.tsx`             |     0 |        0 |     0 |     0 |
| `        TerritoryScopeSelector.tsx`              |     0 |    100.0 | 100.0 |     0 |
| `        useFilteredObjects.tsx`                  |  92.4 |     55.5 |  40.0 |  92.4 |
| `        VitalitySelector.tsx`                    | 100.0 |    100.0 | 100.0 | 100.0 |
| `        WritingSystemFilterSelector.tsx`         |     0 |        0 |     0 |     0 |
| `      sorting`                                   |  55.6 |     90.7 |  85.7 |  55.6 |
| `        sort.tsx`                                |  59.8 |     90.0 |  80.0 |  59.8 |
| `        SortBySelector.tsx`                      |     0 |    100.0 | 100.0 |     0 |
| `        SortDirectionSelector.tsx`               |     0 |    100.0 | 100.0 |     0 |
| `        SortTypes.tsx`                           | 100.0 |    100.0 | 100.0 | 100.0 |
| `    treelist`                                    |  15.2 |    100.0 |     0 |  15.2 |
| `      filterBranch.tsx`                          |   4.2 |    100.0 |     0 |   4.2 |
| `      TreeListNode.tsx`                          |  13.4 |    100.0 |     0 |  13.4 |
| `      TreeListOptions.tsx`                       |  18.4 |    100.0 |     0 |  18.4 |
| `      TreeListPageBody.tsx`                      |  15.2 |    100.0 |     0 |  15.2 |
| `      TreeListRoot.tsx`                          |  26.7 |    100.0 |     0 |  26.7 |
| `  pages`                                         |   3.3 |     60.0 |  54.5 |   3.3 |
| `    AboutPage.tsx`                               |     0 |    100.0 | 100.0 |     0 |
| `    DataPage.tsx`                                |     0 |        0 |     0 |     0 |
| `    DataPageBody.tsx`                            |     0 |    100.0 | 100.0 |     0 |
| `    dataviews`                                   |   7.5 |     50.0 |  42.9 |   7.5 |
| `      DataViews.tsx`                             |     0 |        0 |     0 |     0 |
| `      ViewDetails.tsx`                           |  38.3 |    100.0 |  50.0 |  38.3 |
| `      ViewFamilyTree.tsx`                        |     0 |        0 |     0 |     0 |
| `      ViewMap.tsx`                               |     0 |    100.0 | 100.0 |     0 |
| `      ViewReports.tsx`                           |     0 |    100.0 | 100.0 |     0 |
| `      ViewTable.tsx`                             |     0 |        0 |     0 |     0 |
| `    IntroPage.tsx`                               |     0 |    100.0 | 100.0 |     0 |
| `  shared`                                        |  46.2 |     89.2 |  65.0 |  46.2 |
| `    containers`                                  |  65.0 |    100.0 | 100.0 |  65.0 |
| `      CollapsibleReport.tsx`                     |     0 |    100.0 | 100.0 |     0 |
| `      DetailsField.tsx`                          | 100.0 |    100.0 | 100.0 | 100.0 |
| `      DetailsSection.tsx`                        | 100.0 |    100.0 | 100.0 | 100.0 |
| `      ViewCard.tsx`                              | 100.0 |    100.0 | 100.0 | 100.0 |
| `    hooks`                                       |  19.5 |     60.0 |  42.9 |  19.5 |
| `      useAutoAdjustedWidth.tsx`                  | 100.0 |    100.0 | 100.0 | 100.0 |
| `      useClickOutside.tsx`                       |  87.5 |     80.0 | 100.0 |  87.5 |
| `      useCopyToClipboard.tsx`                    |   1.1 |    100.0 |     0 |   1.1 |
| `      useDebounce.tsx`                           |     0 |        0 |     0 |     0 |
| `      useMediaQuery.tsx`                         |     0 |        0 |     0 |     0 |
| `      usePageBrightness.tsx`                     |     0 |        0 |     0 |     0 |
| `    lib`                                         |  75.8 |     93.5 |  73.7 |  75.8 |
| `      numberUtils.ts`                            |  12.0 |    100.0 |     0 |  12.0 |
| `      PositionInGroup.ts`                        | 100.0 |     85.7 | 100.0 | 100.0 |
| `      setUtils.ts`                               | 100.0 |     94.4 | 100.0 | 100.0 |
| `      stringExportUtils.tsx`                     | 100.0 |     80.0 | 100.0 | 100.0 |
| `      stringUtils.ts`                            |  71.4 |    100.0 |  71.4 |  71.4 |
| `    ui`                                          |  45.1 |     91.3 |  50.0 |  45.1 |
| `      AlignedFraction.tsx`                       |  36.4 |    100.0 |     0 |  36.4 |
| `      CommaSeparated.tsx`                        |  66.7 |     87.5 |  50.0 |  66.7 |
| `      Deemphasized.tsx`                          | 100.0 |    100.0 | 100.0 | 100.0 |
| `      Highlightable.tsx`                         |  94.7 |     92.3 | 100.0 |  94.7 |
| `      LinkButton.tsx`                            |   4.2 |    100.0 |     0 |   4.2 |
| `      LoadingIcon.tsx`                           | 100.0 |    100.0 | 100.0 | 100.0 |
| `      PercentageDifference.tsx`                  |   8.6 |    100.0 |     0 |   8.6 |
| `      Pill.tsx`                                  |  10.5 |    100.0 |     0 |  10.5 |
| `  strings`                                       |     0 |    100.0 | 100.0 |     0 |
| `    EntityTypeDescription.tsx`                   |     0 |    100.0 | 100.0 |     0 |
| `  tests`                                         | 100.0 |    100.0 | 100.0 | 100.0 |
| `    testServer.ts`                               | 100.0 |    100.0 | 100.0 | 100.0 |
| `  widgets`                                       |   8.7 |     46.9 |  31.4 |   8.7 |
| `    cardlists`                                   |  95.7 |     75.0 | 100.0 |  95.7 |
| `      CardList.tsx`                              | 100.0 |     75.0 | 100.0 | 100.0 |
| `      ResponsiveGrid.tsx`                        |  89.7 |     75.0 | 100.0 |  89.7 |
| `    CommonObjectives.tsx`                        |     0 |        0 |     0 |     0 |
| `    controls`                                    |   1.4 |     55.6 |  45.5 |   1.4 |
| `      selectors`                                 |   2.3 |     62.5 |  50.0 |   2.3 |
| `        LanguageSourceSelector.tsx`              |  10.6 |    100.0 |     0 |  10.6 |
| `        LocaleSeparatorSelector.tsx`             |     0 |    100.0 | 100.0 |     0 |
| `        EntityTypeSelector.tsx`                  |     0 |        0 |     0 |     0 |
| `        PageBrightnessSelector.tsx`              |     0 |    100.0 | 100.0 |     0 |
| `        ProfileSelector.tsx`                     |     0 |    100.0 | 100.0 |     0 |
| `        SearchBar.tsx`                           |     0 |        0 |     0 |     0 |
| `        SearchBySelector.tsx`                    |     0 |    100.0 | 100.0 |     0 |
| `        useSearchSuggestions.tsx`                |     0 |        0 |     0 |     0 |
| `        ViewSelector.tsx`                        |     0 |    100.0 | 100.0 |     0 |
| `      SidePanel.tsx`                             |     0 |        0 |     0 |     0 |
| `    CreativeCommonsLicense.tsx`                  |     0 |    100.0 | 100.0 |     0 |
| `    details`                                     |  22.9 |     32.4 |  19.0 |  22.9 |
| `      CensusDetails.tsx`                         |   6.0 |    100.0 |     0 |   6.0 |
| `      LanguageDetails.tsx`                       |  20.0 |    100.0 |     0 |  20.0 |
| `      LocaleDetails.tsx`                         |   9.8 |    100.0 |     0 |   9.8 |
| `      ObjectDetails.tsx`                         |  67.7 |     14.3 | 100.0 |  67.7 |
| `      sections`                                  |  36.7 |     26.7 |  40.0 |  36.7 |
| `        LanguageCodes.tsx`                       |  17.2 |    100.0 |     0 |  17.2 |
| `        LanguageLocation.tsx`                    |  30.4 |    100.0 |     0 |  30.4 |
| `        LanguageNames.tsx`                       |  22.2 |    100.0 |     0 |  22.2 |
| `        TerritoryAttributes.tsx`                 |  74.1 |     30.0 | 100.0 |  74.1 |
| `        TerritoryLocation.tsx`                   |  72.7 |     20.0 | 100.0 |  72.7 |
| `      TerritoryDetails.tsx`                      |  67.7 |     50.0 | 100.0 |  67.7 |
| `      VariantTagDetails.tsx`                     |  15.4 |    100.0 |     0 |  15.4 |
| `      WritingSystemDetails.tsx`                  |   8.2 |    100.0 |     0 |   8.2 |
| `    Loading.tsx`                                 |     0 |    100.0 | 100.0 |     0 |
| `    PageFooter.tsx`                              |     0 |    100.0 | 100.0 |     0 |
| `    PageNavBar.tsx`                              |     0 |    100.0 | 100.0 |     0 |
| `    pathnav`                                     |   7.1 |     38.5 |  14.3 |   7.1 |
| `      getParentsAndDescendants.tsx`              |  22.8 |     55.5 |  33.3 |  22.8 |
| `      ObjectPath.tsx`                            |     0 |        0 |     0 |     0 |
| `      ObjectPathChildren.tsx`                    |     0 |        0 |     0 |     0 |
| `      ObjectPathParents.tsx`                     |     0 |        0 |     0 |     0 |
| `      PathNav.tsx`                               |     0 |        0 |     0 |     0 |
| `    PopulationWarning.tsx`                       |  25.0 |    100.0 |     0 |  25.0 |
| `    reports`                                     |     0 |     20.0 |  20.0 |     0 |
| `      DubiousLanguages.tsx`                      |     0 |        0 |     0 |     0 |
| `      LanguagesLargestDescendant.tsx`            |     0 |        0 |     0 |     0 |
| `      LanguagesWithIdenticalNames.tsx`           |     0 |        0 |     0 |     0 |
| `      PotentialLocales.tsx`                      |     0 |        0 |     0 |     0 |
| `      TableOfCountriesWithCensuses.tsx`          |     0 |    100.0 | 100.0 |     0 |
| `    tables`                                      |   5.4 |     50.0 |  35.0 |   5.4 |
| `      columns`                                   |     0 |     33.3 |  33.3 |     0 |
| `        LanguageCodeColumns.tsx`                 |     0 |        0 |     0 |     0 |
| `        LanguageDigitalSupportColumns.tsx`       |     0 |        0 |     0 |     0 |
| `        LanguageNameColumns.tsx`                 |     0 |        0 |     0 |     0 |
| `        LanguagePopulationColumns.tsx`           |     0 |        0 |     0 |     0 |
| `        LanguageVitalityColumns.tsx`             |     0 |    100.0 | 100.0 |     0 |
| `        LocalePopulationColumns.tsx`             |     0 |    100.0 | 100.0 |     0 |
| `      LanguageTable.tsx`                         |     0 |        0 |     0 |     0 |
| `      LocaleTable.tsx`                           |     0 |    100.0 | 100.0 |     0 |
| `      TableOfAllCensuses.tsx`                    |     0 |    100.0 | 100.0 |     0 |
| `      TableOfLanguagesInCensus.tsx`              |  12.6 |    100.0 |     0 |  12.6 |
| `      TableOfLanguagesInTerritory.tsx`           |  91.2 |     50.0 |  16.7 |  91.2 |
| `      TerritoryTable.tsx`                        |     0 |    100.0 | 100.0 |     0 |
| `      VariantTagTable.tsx`                       |     0 |        0 |     0 |     0 |
| `      WritingSystemTable.tsx`                    |     0 |    100.0 | 100.0 |     0 |
| `    treelists`                                   |   4.3 |     75.0 |  21.4 |   4.3 |
| `      CensusHierarchy.tsx`                       |     0 |    100.0 | 100.0 |     0 |
| `      LanguageHierarchy.tsx`                     |  12.5 |    100.0 |     0 |  12.5 |
| `      LocaleHierarchy.tsx`                       |   6.4 |    100.0 |     0 |   6.4 |
| `      TerritoryHierarchy.tsx`                    |     0 |    100.0 | 100.0 |     0 |
| `      VariantTagHierarchy.tsx`                   |     0 |    100.0 | 100.0 |     0 |
| `      WritingSystemHierarchy.tsx`                |     0 |        0 |     0 |     0 |
