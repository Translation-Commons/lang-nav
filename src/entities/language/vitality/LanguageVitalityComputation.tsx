...
export function getVitalityMetascore(lang: LanguageData): number | undefined {
  const { vitalityEth2013, vitalityEth2025, vitalityISO } = lang;
...
export function getVitalityExplanation(
  ...
  const { vitalityISO, vitalityEth2013, vitalityEth2025 } = lang;

  switch (source) {
    case VitalitySource.ISO:
      if (vitalityISO == null) return <Deemphasized>No vitality data available</Deemphasized>;
      return (
        <div>
          <div>ISO Vitality: {getVitalityISOLabel(vitalityISO)}</div>
          <div>Normalized to a score of {vitalityISO} out of 9.</div>
        </div>
      );
...
export function getVitalityInfo(source: VitalitySource, lang: LanguageData): VitalityInfo {
  switch (source) {
    case VitalitySource.ISO:
      return {
        score: lang.vitalityISO,
        label: getVitalityISOLabel(lang.vitalityISO!),
