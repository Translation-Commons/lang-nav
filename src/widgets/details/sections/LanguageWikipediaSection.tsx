...
const LanguageWikipediaSection: React.FC<{ lang: LanguageData }> = ({ lang }) => {
  ...
  const title = (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
      ...
      {wikipedia && (
        <Pill style={{ color: getStatusColor(wikipedia.status) }}>
          {wikipedia.status}
        </Pill>
      )}
    </div>
  ...
  return (
    <DetailsSection title={title}>
      <div style={{ display: 'flex', gap: '2em', marginTop: 'auto', paddingBottom: '0.5em', justifyContent: 'center' }}>
        {/* Articles */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <div style={{ fontSize: '2em', fontWeight: 700, lineHeight: 1 }}>
            {isActive && wikipedia ? <CountOfPeopleCompact count={wikipedia.articles} /> : <NADisplay />}
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <div style={{ fontSize: '2em', fontWeight: 700, lineHeight: 1 }}>
            {isActive && wikipedia ? <CountOfPeopleCompact count={wikipedia.activeUsers} /> : <NADisplay />}
          </div>
