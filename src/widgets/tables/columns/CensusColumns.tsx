...
function getCensusColumns(): TableColumn<CensusData>[] {
  return [
    ...
    {
      key: 'Languages',
      render: (census) => <CensusLanguageEnumeration census={census} />,
...
const CensusLanguageEnumeration: React.FC<{ census: CensusData }> = ({ census }) => {
  const { getLanguage } = useDataContext();
  const languageNames = Object.keys(census.languageEstimates).map(
    (lang) => getLanguage(lang)?.nameDisplay ?? lang,
  );
  return <HoverableEnumeration items={languageNames} />;
};

export default getCensusColumns;
