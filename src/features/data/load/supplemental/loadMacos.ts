...
/**
 * Load macOS language availability data.
 * File format:
...
export async function loadMacos(
  ...
  await fetch('data/other_sources/macos.tsv')
    .then((res) => res.text())
    ...
    .then((lines) => {
      lines.forEach((line) => {
        ...
        languageCodes.forEach((code) => {
          ...
          if (!language.macos) language.macos = [];
          language.macos.push({ languageCodePath, name, locale, writingSystem });
        });
      });
    })
    .catch((err) => console.error('Error loading macOS data:', err));
}
