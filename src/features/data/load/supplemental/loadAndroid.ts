...
/**
 * Load Android language availability data.
 * File format:
...
export async function loadAndroid(
  ...
  await fetch('data/other_sources/android.tsv')
    .then((res) => res.text())
    ...
    .then((lines) => {
      lines.forEach((line) => {
        ...
        languageCodes.forEach((code) => {
          ...
          if (!language.android) language.android = [];
          language.android.push({ languageCodePath, name, locale, writingSystem });
        });
      });
    })
    .catch((err) => console.error('Error loading Android data:', err));
}
